...
| 6 | Budget Assumptions Generator | 🔲 Planned | Rules file written. Not yet built. See TOOL6_ASSUMPTIONS_RULES.md |
...
**Tool 6:** CLAUDE_CONTEXT.md + TOOL6_ASSUMPTIONS_RULES.md + studiochief_vXX.html
...
Running list of ideas and directions discussed. Not a formal spec — just enough to carry context between sessions.

### Input format expansion (high priority)
- Excel budget upload support — needed for AICP commercial budgets and UK reality budgets (which use multi-tab Excel, not PDF)
- Stripboard upload for feature films (instead of calendar PDF)
- UK multi-tab Excel budget format is its own challenge — tabs for ATL, BTL, Post, Fringes separately

### Production format expansion
- Feature films — architecture already supports it, just needs testing and prompt tuning
- Commercials — AICP budget format is the main technical hurdle
- Branded content
- YouTube / independent vertical producers — underserved market, no tools built for them
- The tools don't know they're reading a reality TV budget. Expansion is mostly prompt work, not rebuilding.

### Future tools being considered
- Schedule Analyzer — upload one-liner or shooting schedule, AI flags production concerns (company moves, turnaround violations, back-to-back nights, location conflicts)
- AI-powered call sheet builder — reads a schedule PDF and drafts the call sheet
- Budget Comparison Tool — upload two budget drafts, AI flags what changed and by how much (useful when network comes back with notes)
- Crew Deal Memo Generator — input deal points, outputs a formatted deal memo (not legal advice, just formatting)

### Business / go-to-market notes
- Launch strategy: soft launch to curated list of former colleagues, freelance LPs/UPMs, ATAS network — not open to everyone immediately
- Free founding member period (~6 months) with email capture to build a list
- White label / licensing conversations: hold until more usage data and at least one strong testimonial
...
- Login and email capture: preview-then-prompt modal at Generate click
- Visual polish pass needed on Tax Calculator and Cost & Variance Report
- Whether the suite could eventually serve adjacent industries (construction, events) — probably not a priority given entertainment verticals are the more natural expansion
